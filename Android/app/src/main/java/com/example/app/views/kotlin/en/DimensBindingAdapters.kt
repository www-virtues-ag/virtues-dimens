...
object DimensBindingAdapters {
    ...
    fun setDynamicWidth(view: View, dpValue: Float) {
        ...
        val pxValue = VirtuesDimens.dynamic(dpValue).toPx(view.resources)
     ...
     * VirtuesDimensDynamic.toSp() ensures that the scaling adjustment is applied to the text.
     * XML usage: app:dynamicTextSizeDp="@{20f}"
     *
     * [PT] Define o tamanho do texto (TextView), convertendo Float Dp para SP/PX dinâmico.
     * VirtuesDimensDynamic.toSp() garante que o ajuste de escala seja aplicado ao texto.
    ...
    fun setDynamicTextSize(textView: TextView, dpValue: Float) {
        ...
        val spValueInPx = VirtuesDimens.dynamic(dpValue).toSp(textView.resources)
